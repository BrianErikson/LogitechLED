...
#define USB_VENDOR_ID	0x046d 	// Logitech, Inc
#define USB_G810_ID		0xc331 	// G810

/* table of devices that work with this driver */
static const struct usb_device_id table[] = {
	{ USB_DEVICE(USB_VENDOR_ID, USB_G810_ID) },
	{ }					/* Terminating entry */
};
...
#define to_dev(d) container_of(d, struct usb_skel, kref)

static struct usb_driver driver;
static void draw_down(struct usb_skel *dev);

static void delete(struct kref *kref)
{
	struct usb_skel *dev = to_dev(kref);
...
static int __init init_logitech_led(void) {
	printk(KERN_INFO "Hello World\n");

	return 0;
}

static void __exit cleanup_logitech_led(void) {
	printk(KERN_INFO "Goodbye World\n");
}

module_init(init_logitech_led);
module_exit(cleanup_logitech_led);
